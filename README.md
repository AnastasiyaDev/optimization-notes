...
Лекция на эту тему [Аппаратное ускорение в жизни верстальщика](https://video.yandex.ru/users/ya-events/view/3267/)
...
* **FOIT (Flash of Invisible Text)** — «мелькание невидимого текста», когда во время загрузки веб-шрифта текст не отображается, изначально загружается невидемый шрифт;
...
Если используются сторонние поставщики шрифтов на ваш сайт, то у вас нет возможности контроливать css и следовательно сделать так, как описано выше, но можно это сделать с помощью **font-loading API** через js.

В чем суть этого подхода:
1. В файле со стилями, после объявления шрифтов, надо указать семейство шрифтов для конкретного селектора в области класса `.fonts-loaded`
```
.fonts-loaded body{
